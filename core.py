    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                print(f"[ERREUR] Ligne joueur ignorée : {e}")
                continue
...
# === ÉVALUATION CONDITION (déjà parfait) ===
...
def assign_rewards(players: List[Dict], rules: List[Dict]) -> List[Dict]:
    ...
    for rule in rules:
        cat = rule['CATEGORIE']
        prizes = [p.strip() for p in rule['PRIX'].split('|')]
        cond = rule.get('CONDITION', 'none').strip()
        rank_str = rule['RANG'].strip().lower()
        order = int(rule.get('ORDRE', 999))
        ...
        if rank_str == 'best':
            if eligible:
                ...
                Ascending = True
                awards.append({
                    'category': cat, 'place': prizes[0], 'player': w['name'],
        ...
        for i, w in enumerate(winners):
            ...
            awards.append({
                'category': cat, 'place': place_prize, 'player': w['name'],
